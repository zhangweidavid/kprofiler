...
package com.kaola.kprofiler.javassist.tools.rmi;

import com.kaola.kprofiler.javassist.*;
import java.lang.reflect.Method;
import java.util.Hashtable;
import com.kaola.kprofiler.javassist.CtMethod.ConstParameter;
...
public class StubGenerator implements Translator {
	...
	private CtClass[] toCtClass(Class[] rtclasses)
			throws NotFoundException {
	...
	/*
	 * ms must not be an array of CtMethod. To invoke a method ms[i] on a server, a client must send i to the server.
	 */
		...
		for (int i = 0; i < ms.length; ++i) {
			...
			if (m.getDeclaringClass() != Object.class && !Modifier.isFinal(mod))
				if (Modifier.isPublic(mod)) {
					...
					wmethod = CtNewMethod.wrapped(toCtClass(m.getReturnType()), m.getName(),
							toCtClass(m.getParameterTypes()), exceptionForProxy, body, ConstParameter.integer(i),
							proxy);
